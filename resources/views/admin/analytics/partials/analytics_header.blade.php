        ...
        $(function () {
            function getMonthlySummary()
            {
                doAjax('/api/analytics/visitors', null, function (response) {
                   $('#visitors').html(response['datasets'][0]['data'][0]);
                    doughnutChart('chart-visitors', response);
                });

                doAjax('/api/analytics/unique-visitors', null, function (response) {
                    $('#unique-visitors').html(response['datasets'][0]['data'][0]);
                    doughnutChart('chart-unique-visitors', response);
                });

                doAjax('/api/analytics/bounce-rate', null, function (response) {
                    response['datasets'][0]['data'][0] = parseFloat(response['datasets'][0]['data'][0]).toFixed(2);
                    response['datasets'][0]['data'][1] = parseFloat(response['datasets'][0]['data'][1]).toFixed(2);
                    $('#bounce-rate').html(response['datasets'][0]['data'][0] + '<sup style="font-size: 20px">%</sup>');
                    doughnutChart('chart-bounce-rate', response);
                });

                doAjax('/api/analytics/page-load', null, function (response) {
                    $('#page-load').html(parseFloat(response).toFixed(0) + '<sup style="font-size: 20px">sec</sup>');
                });

                doAjax('/api/analytics/active-visitors', null, function (response) {
                    $('#page-active-visitors').html(parseFloat(response).toFixed(0));
                });
            }
