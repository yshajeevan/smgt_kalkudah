<div id="messages">
    <a class="nav-link dropdown-toggle" href="#" id="messagesDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        ...
        <!-- Counter - Messages -->
        @if(count(Helper::activitylog())>10)
            <span data-count="10" class="badge badge-danger badge-counter">10+</span>
        @else 
            <span data-count="{{count(Helper::activitylog())}}" class="badge badge-danger badge-counter">{{count(Helper::activitylog())}}</span>
        @endif
    </a>
    <!-- Dropdown - Messages -->
    <div class="dropdown-list dropdown-menu dropdown-menu-right shadow animated--grow-in" aria-labelledby="messagesDropdown">
        ...
        <div id="message-items">
            @foreach(Helper::activitylog() as $log)
                <a class="dropdown-item d-flex align-items-center" href="#">
                    <div class="dropdown-list-image mr-3">
                        @if($log->subject_id > '100')
                    ...
                    <div class="font-weight-bold">
                        <div class="small text-gray-500">{{$log->description}}</div>
                    </div>
                </a>
                @if($loop->index+1==10) 
                  @php 
        ...
        <a class="dropdown-item text-center small text-gray-500" href="{{route('all.notification')}}">Read More Notifications</a>
    </div>
</div>
